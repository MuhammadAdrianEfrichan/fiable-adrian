import partnerSatu from "../assets/image/timKami/PartnerProfile/partnerSatu.png"
import partnerDua from "../assets/image/timKami/PartnerProfile/partnerDua.png"
import consultanSatu from "../assets/image/timKami/consultan/consultanSatu.png"
import consultanDua from "../assets/image/timKami/consultan/consultanDua.png"
import lawyerSatu from "../assets/image/timKami/lawyer/lawyerSatu.png"
import lawyerDua from "../assets/image/timKami/lawyer/lawyerdua.png"


export const teamCategories = [
  {
    id: "partner",
    title: "Partner Profile",
    member: [
      {
        id: 1,
        name: "Kevin Pratama Putra, S.H.",
        role: "Managing Partner",
        specialty: "Litigator",
        bio: "Kevin Pratama Putra adalah pendiri dan mitra manajemen dari Fiable Law Office, telah menapaki perjalanan karier yang menginspirasi. Setelah meraih gelar dari Fakultas Hukum Universitas Andalas, Kevin memulai langkahnya sebagai Associates Lawyer di Kantor Hukum Endang Suparta & Partners yang dikenal sebagai salah satu kantor hukum terbaik di Pekanbaru, Riau. Selain itu, ia juga mengejar pengalaman berharga sebagai Pengacara Pos Pelayanan Hukum di Pengadilan Negeri Pekanbaru. Dari kedua pengalaman ini, Kevin membawa dedikasi dan keahlian yang luar biasa dalam memimpin Fiable Law Office menuju kesuksesan yang lebih besar lagi.",
        photo: partnerSatu,
        instagramUrl: "https://instagram.com/...",
        linkedinUrl: "https://linkedin.com/in/...",
        portfolioUrl: "/portfolio/kevin.pdf",
      },
      {
        id: 2,
        name: "Muhammad Arief Azmi, S.H.",
        role: "Partner",
        specialty: "Litigator",
        bio: "Kevin Pratama Putra adalah pendiri dan mitra manajemen dari Fiable Law Office, telah menapaki perjalanan karier yang menginspirasi. Setelah meraih gelar dari Fakultas Hukum Universitas Andalas, Kevin memulai langkahnya sebagai Associates Lawyer di Kantor Hukum Endang Suparta & Partners yang dikenal sebagai salah satu kantor hukum terbaik di Pekanbaru, Riau. Selain itu, ia juga mengejar pengalaman berharga sebagai Pengacara Pos Pelayanan Hukum di Pengadilan Negeri Pekanbaru. Dari kedua pengalaman ini, Kevin membawa dedikasi dan keahlian yang luar biasa dalam memimpin Fiable Law Office menuju kesuksesan yang lebih besar lagi.",
        photo: partnerSatu,
        instagramUrl: "...",
        linkedinUrl: "...",
        portfolioUrl: "/portfolio/arief.pdf",
      },
      {
        id: 3,
        name: "Vino Oktavia, S.H., M.H.",
        role: "",
        specialty: "Environmental Resources Lawyer & Customary Land",
        bio: "Vino Oktavia menyelesaikan pendidikan Sarjana dan Pascasarjana pada Fakultas Hukum Universitas Andalas dengan Predikat Summa Cumlaude, Vino merupakan seorang Pengacara Senior yang telah mempunyai sertifikasi sebagai Mediator oleh Mahkamah Agung RI. Pengalaman berharga Vino juga meliputi peran sebagai Dosen Luar Biasa di UIN Imam Bonjol Padang, dimana pengetahuan hukumnya dibagikan dengan generasi muda. Selain itu, dalam perannya sebagai Direktur Lembaga Bantuan Hukum (LBH) Padang, Vino telah berhasil memberikan kontribusi penting dalam memberikan akses keadilan kepada mereka yang membutuhkan. Dengan integritas dan komitmennya terhadap keadilan, Vino Oktavia melanjutkan perjalanan karirnya sebagai mitra manajemen di Fiable Law Office.",
        photo: partnerDua,
        instagramUrl: "...",
        linkedinUrl: "...",
        portfolioUrl: "/portfolio/vino.pdf",
      },
    ],
  },
  {
    id: "legalConsultan",
    title: "Legal Consultant Profile",
    members: [
        {
        id: 1,
        name: "Andre Fitra Anwar, S.H",
        specialty: " General Corporate & Commercial",
        bio: "Andre Fitra Anwar adalah mitra legal manajemen dari Fiable Law Office. Setelah meraih gelar dari Fakultas Hukum Universitas Andalas, Andre memulai langkah karirnya sebagai Asisten Pengacara Publik di Lembaga Bantuan Hukum Jakarta (LBH Jakarta), serta sebagai Staf Hukum di Yayasan Lembaga Konsumen Indonesia (YLKI).",
        bioDua : "Namun, keinginan untuk terlibat dalam tantangan yang lebih besar membawa Andre ke dunia bisnis startup. Andre melanjutkan perjalanan karirnya sebagai Head of Legal di BenihBaik.com, sebuah startup crowdfunding yang didirikan oleh Andy F Noya, seorang wartawan senior dan host Kick Andy yang terkenal di Metro Tv.",
        photo: consultanSatu,
        instagramUrl: "...",
        linkedinUrl: "...",
        portfolioUrl: "/portfolio/vino.pdf",
      },
        {
        id: 2,
        name:"Giffary Syarlas, S.H., M.H.",
        specialty: " General Corporate & Commercial",
        bio: "Giffary Syarlas adalah legal consultant manajemen dari Fiable Law Office, telah menjalani perjalanan karier yang menginspirasi. Setelah meraih gelar dari Fakultas Hukum Universitas Andalas dan Magister Hukum di Universitas Indonesia, Giffar memulai langkahnya sebagai junior associate di Dams & Co, Counsellors at Law. Di kantor tersebut, ia aktif menangani berbagai isu legal di bidang korporat, kepatuhan, transaksi bisnis, dan Initial Public Offering (IPO) di Bursa Efek. Dengan pengalaman yang luas dan dedikasinya, Giffary terus membawa warna baru dalam memberikan layanan hukum yang unggul bersama Fiable Law Office..",
        bioDua : "",
        photo: consultanDua,
        instagramUrl: "...",
        linkedinUrl: "...",
        portfolioUrl: "...",
      },
    ],
  },
  {
    id: "lawyer",
    title: "Lawyer Profile",
    members: [
      {
        id: 1,
        name:"Mirza Ardila, S.H.",
        specialty: "Family Law Litigator",
        photo: lawyerSatu,
        instagramUrl: "...",
        linkedinUrl: "...",
        portfolioUrl: "...",
      },
       {
        id: 2,
        name:"Mirza Ardila, S.H.",
        specialty: "Family Law Litigator",
        photo: lawyerDua,
        instagramUrl: "...",
        linkedinUrl: "...",
        portfolioUrl: "...",
      },
    ],
  },
];